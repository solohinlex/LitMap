...
`ask --context` извлекает релевантный контекст (чанки) и выводит его в stdout без
вызова чат-модели — удобно, когда ответ формулирует внешняя модель (например,
через скилл opencode), а litmap используется только для поиска и эмбеддингов:

```bash
litmap -p severny ask "где Иван теряет кольцо?" --context
```
